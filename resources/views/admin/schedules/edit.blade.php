...
<div class="animate-fade-up" style="max-width: 720px;">
    ...
    <div style="margin-bottom: 1.5rem;">
        ...
        <h2 class="text-heading">Edit Jadwal Praktikum</h2>
    </div>

    <div class="card" style="padding: 1.5rem;">
        <form action="{{ route('admin.schedules.update', $schedule) }}" method="POST" style="display: flex; flex-direction: column; gap: 1.25rem;">
            @csrf
            @method('PUT')
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.25rem;">
                ...
                <div>
                    ...
                    <input type="text" name="angkatan" id="angkatan" value="{{ old('angkatan', $schedule->angkatan) }}" class="form-input" placeholder="2023" required>
                </div>
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.25rem;">
                <div>
                    ...
                    <select name="program" id="program" class="form-input" required>
                        <option value="Reguler" {{ old('program', $schedule->program) === 'Reguler' ? 'selected' : '' }}>Reguler</option>
                        <option value="Ekstensi" {{ old('program', $schedule->program) === 'Ekstensi' ? 'selected' : '' }}>Ekstensi</option>
                    </select>
                </div>
                <div>
                    <label class="form-label" for="kelas">Kelas</label>
                    <input type="text" name="kelas" id="kelas" value="{{ old('kelas', $schedule->kelas) }}" class="form-input" placeholder="TI.23.A.1" required>
                </div>
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 1.25rem;">
                ...
                <div>
                    ...
                    <input type="text" name="waktu_mulai" id="waktu_mulai" value="{{ old('waktu_mulai', $schedule->waktu_mulai) }}" class="form-input" placeholder="08.00" required>
                </div>
                <div>
                    <label class="form-label" for="waktu_selesai">Waktu Selesai</label>
                    <input type="text" name="waktu_selesai" id="waktu_selesai" value="{{ old('waktu_selesai', $schedule->waktu_selesai) }}" class="form-input" placeholder="10.00" required>
                </div>
            </div>

            <div>
                <label class="form-label" for="dosen">Dosen</label>
                <input type="text" name="dosen" id="dosen" value="{{ old('dosen', $schedule->dosen) }}" class="form-input" placeholder="Nama dosen pengampu" required>
            </div>

            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.25rem;">
                <div>
                    <label class="form-label" for="ruangan">Ruangan</label>
                    <input type="text" name="ruangan" id="ruangan" value="{{ old('ruangan', $schedule->ruangan) }}" class="form-input" placeholder="Lab 1">
                </div>
            ...
            <div>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-top: 4px;">
                    @php
                        $selectedAssistants = old('assistants', $schedule->assistants->pluck('id')->toArray());
                    @endphp
                    @foreach($assistants as $assistant)
                        <label style="display: flex; align-items: center; gap: 10px; padding: 10px 14px; cursor: pointer; border: 1px solid var(--c-border); border-radius: var(--radius-md); background: var(--c-surface); transition: border-color 0.15s;">
                            <input type="checkbox" name="assistants[]" value="{{ $assistant->id }}" {{ in_array($assistant->id, $selectedAssistants) ? 'checked' : '' }}>
                            <div>
            ...
            <div>
                ...
                <textarea name="catatan" id="catatan" rows="2" class="form-input" placeholder="Catatan tambahan...">{{ old('catatan', $schedule->catatan) }}</textarea>
            </div>
